...
// `telescope_rollups`, and `telescope_schema_meta` (see
 ...
 *
 * DERIVED from the three entities' own `tableName` — never a hardcoded
...
export function telescopeManagedTables(): string[] {
  return [TelescopeEntry.tableName, TelescopeRollup.tableName, TelescopeSchemaMeta.tableName];
}
