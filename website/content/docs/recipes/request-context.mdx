...
async function bootstrap() {
  ...
  // Exclude Telescope's own routes from the prefix — otherwise the dashboard,
  // its API, and its assets get shifted under /api/telescope/... too.
...
- **`telescopeMountPaths(path = 'telescope')`** solves the *other* half of the same prefix problem — not capture, but the dashboard's own routes. `setGlobalPrefix` rewrites every controller route, including Telescope's own (dashboard, API, assets) unless you exclude them; this helper returns the route roots to exclude, normalized the same way `TelescopeModule.forRoot({ path })` normalizes its mount path — so passing a custom `path` here matches a custom `path` there. Needed only when you call `setGlobalPrefix` at all; skip it if your app doesn't.
