...
| `guards` / `imports` | Bring-your-own `CanActivate` guard(s) fronting the console (appended to the built-in gate above) — see [Securing the console](/docs/dashboard/auth#securing-the-console-with-your-own-guards). |
...
| `mcp` | Optional MCP server for coding agents — `true` (dev-only) or `{ token }`. Disabled by default. See [MCP server](/docs/concepts/mcp). |
| `overloadProtection` | Pause capture under event-loop pressure. `true` (default, 200ms p99), `false`, or `{ maxEventLoopLagMs }`. See [Performance](/docs/concepts/performance#overload-protection--pause-under-event-loop-pressure). |
